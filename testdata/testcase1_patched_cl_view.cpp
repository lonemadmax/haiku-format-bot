...
TOffscreenView::TOffscreenView(BRect frame, const char* name, short mRadius, short hRadius,
...
dummy_label:
	if (error == B_OK) {
		...
		for (short loop = 0; loop <= 8; loop++) {
			if ((picHelp = rsrcs.FindResource('PICT', loop + 4, &len))) {
				fClockFace[loop] = new BBitmap(theRect, B_CMAP8);
				fClockFace[loop]->SetBits(picHelp, len, 0, B_CMAP8);
				free(picHelp);
			}
		}

		theRect.Set(0, 0, 15, 15);
		if ((picHelp = rsrcs.FindResource(B_MINI_ICON_TYPE, "center", &len))) {
			fCenter = new BBitmap(theRect, B_CMAP8);
			fCenter->SetBits(picHelp, len, 0, B_CMAP8);
			free(picHelp);
		}

		theRect.Set(0, 0, 2, 2);
		if ((picHelp = rsrcs.FindResource('PICT', 13, &len))) {
			fInner = new BBitmap(theRect, B_CMAP8);
			fInner->SetBits(picHelp, len, 0, B_CMAP8);
			free(picHelp);
		}
	}
...
TOnscreenView::TOnscreenView(BRect rect, const char* title, short mRadius, short hRadius,
	short offset)
	: BView(rect, title, B_FOLLOW_NONE, B_WILL_DRAW | B_PULSE_NEEDED | B_DRAW_ON_CHILDREN),
	  fOffscreen(NULL),
	  fOffscreenView(NULL)
{
...
void
TOnscreenView::InitObject(BRect rect, short mRadius, short hRadius, short offset, long face,
	bool show)
{
...
TOnscreenView::TOnscreenView(BMessage* data)
	: BView(data),
	  ...
	  fOffscreenView(NULL)
{
	InitObject(data->FindRect("bounds"), data->FindInt32("mRadius"), data->FindInt32("hRadius"),
		data->FindInt32("offset"), data->FindInt32("face"), data->FindBool("seconds"));
}
...
TOnscreenView::MessageReceived(BMessage* msg)
{
	switch (msg->what) {
		...
		{
			BAlert* alert = new BAlert(B_TRANSLATE("About Clock"),
				B_TRANSLATE("Clock (The Replicant version)\n\nCopyright 2002-2020 "
